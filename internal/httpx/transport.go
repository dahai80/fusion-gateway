...
// N2 (audit): shared HTTP transport + bounded-reader helpers extracted from
// internal/adapter so non-adapter packages (cluster, middleware, hardware,
// router) depend on a leaf httpx package instead of reaching INTO adapter
// (the provider layer). The prior edge cluster -> adapter was a reverse
// dependency: cluster imported adapter.TransportForBackend /
// LimitResponseReader, coupling the cluster discovery layer to the provider
// layer. Moving the leaf transport/reader helpers to httpx makes both
// adapter and cluster import the same leaf — no reverse edge.
//
// Behavior is identical to the prior adapter helpers (same constants, same
// resolve logic, same logs). Only the package path changed.
...
// message. Bounded replacement for the bare io.ReadAll(resp.Body) pattern on
// non-200 paths: an unbounded read lets a hostile or buggy upstream exhaust
// gateway memory by streaming gigabytes on an error status. The cap is
...
// DefaultMaxConnsPerHost re-exports defaultMaxConnsPerHost for cross-package
// test assertions (adapter transport_test.go verifies the cap applied).
const DefaultMaxConnsPerHost = defaultMaxConnsPerHost
...
// DefaultResponseHeaderTimeout re-exports defaultResponseHeaderTimeout for
// cross-package test assertions (adapter r5_test.go verifies the R5 timeout).
const DefaultResponseHeaderTimeout = defaultResponseHeaderTimeout
